...
export function createBrowserHelia(): Promise<any> {
  ...
  initializationPromise = (async () => {
    ...
    const pool =
      (relayManager as any).relayPool ||
      peersConfig.relays.slice(0, CONFIG.RELAY_POOL_SIZE);
    ...
    while (!heliaNode && currentRelayIndex < pool.length) {
      ...
      console.log(
        `🚀 [HeliaInit] Пробуем запустить ноду через релей: ${relay.name || relay.peerId.slice(-6)}`,
      );
      ...
      try {
        ...
        console.log(
          `✅ [HeliaInit] Успешный коннект к релею: ${relay.name || relay.peerId.slice(-6)}`,
        );
        ...
        console.warn(
          `⚠️ [HeliaInit] Не удалось подключиться к релею ${relay.name || relay.peerId.slice(-6)}. Ошибка: ${error.message}`,
        );
    ...
    if (!heliaNode) {
      throw new Error(
        '🚨 [HeliaInit] ФАТАЛЬНАЯ ОШИБКА: Ни один релей из пула не доступен!',
      );
    }
